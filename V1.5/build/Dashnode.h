...
{ \
	char buff[80]; \
	sprintf( buff, " 17 %s %d %d\n", mem, index, section ); \
	m_pParentEntity->sim_trace( 'R', stage, 3, buff ); \
}
...
class string_array
{
  ...
  string_array(int x);
string_array();
  string_array(int size, const char* name);
char* operator[](int);
...
parameter_value_array();
  parameter_value_array(int size, const char* name);
int& operator[](int);
...
class cache_memory
{
  ...
  cache_memory(int x);
cache_memory();
  cache_memory(int size, const char* name);
...
class memory_trace
{
  ...
  memory_trace(int x);
memory_trace();
  memory_trace(int size, const char* name);
...
void getmem_trace_structFromType(Type *t, mem_trace_struct* pmem_trace_struct );
  void startup();
  void report();
  void body();
private:
...
void getca_line_structFromType(Type *t, ca_line_struct* pca_line_struct );
  void startup();
  void report();
  void body();
private:
...
extern void PutValue( char* buff, const char* val );
