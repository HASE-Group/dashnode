...
int stage = 0;
char type = 'P';  char buf[255];
  sprintf( buf," 0 ");
  strcat( buf, "\n" );
  S->trace(type, stage, -1, buf);
...
void PutValue( char* buff, const char* val )
{
	size_t len = strlen(buff);
	size_t i=0;
	for( ; val[i]!='\0'; i++)
		buff[len+i]=(val[i]==' ')?'_':val[i];
	buff[len+i]=' ';
	buff[len+i+1]='\0';
}
...
void send_DATAPKT( sim_port &dest, p1_struct val, sim_time delay ,int colour, int stage) {
  ...
  if (colour>0) {
    char buff[250];
    sprintf(buff," %d %s DATAPKT ", colour,dest.get_pname());
    PutValue(buff, val);
    strcat( buff, "\n");
...
string_array::string_array(int size, const char* name)
{
  section=0;
  max=size;
...
parameter_value_array::parameter_value_array(int size, const char* name)
{
  section=0;
  max=size;
  InstanceName=new char[strlen(name)+2];
...
cache_memory::cache_memory(int size, const char* name)
{
  section=0;
  max=size;
  InstanceName=new char[strlen(name)+2];
...
memory_trace::memory_trace(int size, const char* name)
{
  section=0;
  max=size;
  InstanceName=new char[strlen(name)+2];
