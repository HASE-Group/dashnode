...
#include <ArrayManager.h>
  int mips::MIPS_delay = 1;

 void mips::getmem_trace_structFromType(Type *t, mem_trace_struct *mem_trace_structInstance) {
  mem_trace_structInstance->mt_address = ((Int *)((Struct *)t)->getField(0))->intValue();
 ((Struct *)t)->getField(1)->toString(mem_trace_structInstance->mt_rw);
 ((Struct *)t)->getField(2)->toString(mem_trace_structInstance->mt_id);
}
// --------------------------------------------------------
// mips Constructor
// --------------------------------------------------------
mips::mips(char*n , int mem_trace_size_i, int TC_i) :
  sim_entity(n),
  to_p_cache(n),
  from_p_cache(n),
  ...
  TC(TC_i)
{
 ...
 String __char("(undefined)", ' ');
  ...
  if (__mem_traceTotal == -1) printf("No memory file found for %s.mem_trace.\n", get_name());
  else if (__mem_traceTotal < 0) printf("%s:Error reading memory mem_trace, line %d.\n", get_name(), -(__mem_traceTotal + 2));
  else {
...
char* mips::state_names[] = {"M_STOPPED ","M_RUNNING ","M_WAITING "};
// --------------------------------------------------------
